...
class Db:
    ...
    Usage:
        ...
        - to listen to signals use any list methods, e.g.

        ..code-block:: python
            service.on_db_obj_created.append(my_listener_coro)

        - to send a signal use coroutine send, e.g.

        ..code-block:: python
            await service.on_db_obj_created.send(*args, **kwargs)
    """
    def __init__(self):
        self.pool = (asyncpg.create_pool(dsn=DSN, command_timeout=60))
        self.tables = {}
    ...
    async def shutdown(self):
        ...
        delattr(self.service, 'db')

    def create_tables(self, models: (Iterator, Generator)) -> dict:
        r"""
    ...
    async def drop_tables(self, tables: (list, Iterator)) -> bool:
        r"""
        Drop tables from the database.
        :param tables: a list or tuple of table names
        ...
        try:
            async with self.pool.acquire() as connection:
                for name in tables:
                    connection.execute(f"DROP TABLE {name} CASCADE")
    ...
    async def create(self, connection: Connection, table: str, *,
        ...
        :param data: key: value pairs where keys are column names and values the
    ...
    async def update(self,
        ...
        :return: updated row as dict containing all columns or those
        specified by
            kwargs['returning']
        """
    ...
    async def delete(self, connection, table: str, *, o_id: int = None) -> int \
                                                                           or \
                                                                           None:
        r"""
    ...
    async def get(self, connection: Connection, table: str, *, where: str =
    None,
    ...
    async def get_by_id(self, connection: Connection, table: str, *,
                        o_id: int =
                        None,
                        **kwargs) -> dict or None:
        r"""
        ...
        :return: the matching row as dict containing all columns or those
        specified by kwargs['fields']
        """
        fields = kwargs.get('fields', '*')
        if len(fields) > 1 and not isinstance(fields, (list, tuple)):
            raise TypeError(f"<fields> must be type list or tuple")
