...
from bonham.core.models import Base
...
__all__ = [
    'get_alchemy_engine',
    'setup', 'shutdown', 'ForeignKey', 'create_db', 'drop_db', 'create_tables', 'drop_tables',
           'create', 'update', 'delete', 'get', 'get_by_id', 'get_by_key']

def get_alchemy_engine(dsn):
    return sa.create_engine(dsn, client_encoding='utf8', echo=True)
...
async def setup(app):
    ...
    """
    app['pg_db'] = await asyncpg.create_pool(
        dsn=app['config']['dsn'], command_timeout=60)
    app.tables = {}


async def shutdown(app):
    """ Gracefully shutdown all open connections to te database and close the connection pool."""
    await app['pg_db'].close()
...
async def drop_tables(connection, tables: (list, tuple)) -> bool:
    """
    ...
    try:
        for table in tables:
            await connection.execute(f"DROP TABLE {table} CASCADE")
        return True
...
class ForeignKey(sa.Column):
    def __init__(self, related: str, **kwargs):
        ...
        super().__init__(
            sa.Integer,
            sa.ForeignKey(f"{related}.id", ondelete=ondelete, onupdate=onupdate),
