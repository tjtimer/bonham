...
async def create_calendar(request):
    ...
    try:
        ...
        calendar_serialized = await serialize('calendars', calendar)
...
async def get_calendars(request):
    calendars = await Calendar().get(request['connection'])
    async with request['connection'].transaction():
        for calendar in calendars:
            calendar['type'] = await Calendar().get_by_id(request['connection'], object_id=calendar['type'])
            calendar['owner'] = await Account().get_by_id(request['connection'], object_id=calendar['owner'])
